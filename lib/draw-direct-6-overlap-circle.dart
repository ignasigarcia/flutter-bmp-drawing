...
class DrawDirect6OverlapCircle extends StatefulWidget {
  final int width;
  final int height;
  final int maxPixels;

  const DrawDirect6OverlapCircle({Key key, this.width, this.height, this.maxPixels}) : super(key: key);

  @override
  _DrawState createState() => _DrawState();
}

class _DrawState extends State<DrawDirect6OverlapCircle> {
  Int32List pixels;
  Point lastPoint;
  int color;
  int thickness = 1;
  ...
  setPixel(x, y) {
    // TODO not cool
    pixels[y.toInt() * widget.width + x.toInt()] = color;
  }
                  ...
                  GestureDetector(
                    ...
                    onPanUpdate: (details) {
                      ...
                      if (x > 0 && y > 0 && y * widget.width + x <= widget.maxPixels && lastPoint != null) {
                        ...
                        plotCircleAA(x, y, thickness);
                        plotCircle(x, y + 100, thickness);
