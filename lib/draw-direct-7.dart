...
class DrawDirect7 extends StatefulWidget {
  final int width;
  final int height;
  final int maxPixels;

  const DrawDirect7({Key key, this.width, this.height, this.maxPixels}) : super(key: key);

  @override
  _DrawState createState() => _DrawState();
}

class _DrawState extends State<DrawDirect7> {
  Int32List pixels;
  Point lastPoint;
  int color;
  double thickness = 2;
                  ...
                  GestureDetector(
                    ...
                    onPanUpdate: (details) {
                      ...
                      if (x > 0 && y > 0 && y * widget.width + x <= widget.maxPixels && lastPoint != null) {
                        ...
                        // List<Point> nextPoints = bresenham(lastPoint.x, lastPoint.y, x, y);
                        // // plotCircleAA(x, y, 1);
                        // try {
                        //   nextPoints.forEach((point) => plotCircleAA(point.x.toInt(), point.y.toInt(), thickness));
                        // } catch (e) {
                        //   var tas = 1;
                        // }
